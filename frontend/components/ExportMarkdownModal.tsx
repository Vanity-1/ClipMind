...
import { ApiError, knowledgeApi, Video, IngestStreamEvent } from "@/lib/api";
...
export default function ExportMarkdownModal({
  ...
  const ingestAndDownload = async () => {
    ...
    try {
      let failed = false;
      await knowledgeApi.ingestVideoStream(
        video.bvid,
        folderId,
        sessionId,
        (evt) => {
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={cancel}>
        ...
        {pendingMode ? (
          <div className="export-ingest-confirm">
            ...
            {ingestSteps && ingestSteps.length > 0 && (
              <div className="export-ingest-steps" aria-live="polite">
                {ingestSteps.map((s, i) => {
                  const tone =
                    s.event === "error" ? "error"
                      : s.event === "done" ? "done"
                      ...
                      : "running";
                  return (
                    <div key={i} className={`export-ingest-step tone-${tone}`}>
                      <span className="export-ingest-step-icon">
                        {tone === "done" ? "✓" : tone === "error" ? "✗" : "•"}
                      </span>
                      <span className="export-ingest-step-msg">{s.message}</span>
                    </div>
                  );
                })}
              </div>
